...
class MessageController extends Controller
{
    ...
    {
        $messages = Message::all(); // Get all messages regardless of user authentication
     ...
     *         @OA\JsonContent(
     *             @OA\Property(property="contenu", type="string", example="Contenu du message"),
     *             @OA\Property(property="user_id", type="integer", example=1), // User ID can be optional
     *             @OA\Property(property="statut_lecture", type="string", example="non lu")
     *         )
     *     ),
    ...
    public function store(StoreMessageRequest $request)
    {
        ...
        $message = Message::create(array_merge($request->validated(), [
            'statut_lecture' => 'non lu', // Default status
            'user_id' => $user->id // Set to the authenticated user's ID
        ]));
     ...
     *     @OA\RequestBody(required=true,
     *         @OA\JsonContent(
     *             @OA\Property(property="contenu", type="string", example="Contenu mis à jour du message")
     *         )
     *     ),
